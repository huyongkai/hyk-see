...
export class TransportData {
  ...
  getAuthId() {
    if (typeof this.getUserId === "function") {
      ...
      if (typeof id === "string" || typeof id === "number") {
        ...
        console.error(
          `useId：${id} 期望返回 string 或 number 类型，实际返回值为：${typeof id}`
        );
      }
    }
    return "";
  ...
  bindOptions(initOptions) {
    const { dsn, apikey, beforeDataReport, userId, getUserId, useImgUpload } =
      initOptions;
    validateOptions(dsn, "dsn", "string") && (this.errorDsn = dsn);
    validateOptions(apikey, "apikey", "string") && (this.apikey = apikey);
    validateOptions(beforeDataReport, "beforeDataReport", "function") &&
      (this.beforeDataReport = beforeDataReport);
    validateOptions(userId, "userId", "string") && (this.userId = userId || "");
    validateOptions(getUserId, "getUserId", "function") &&
      (this.getUserId = getUserId);
    validateOptions(useImgUpload, "useImgUpload", "boolean") &&
